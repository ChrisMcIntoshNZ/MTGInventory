        ...
        public async System.Threading.Tasks.Task setGroupIDsAsync()
        {
            using (var httpClient = new HttpClient())
            {
                ...
                while (offset < limit)
                {
                    ...
                    using (var request = new HttpRequestMessage(new HttpMethod("GET"), "https://api.tcgplayer.com/v1.32.0/catalog/groups?categoryId=1&limit=100&offset=" + offset))
                    {
    ...
    class Card
    {
        ...
        public async System.Threading.Tasks.Task setPricesAsync()
        {
            using (var httpClient = new HttpClient())
            {
                ...
                using (var request = new HttpRequestMessage(new HttpMethod("GET"), "https://api.tcgplayer.com/v1.32.0/pricing/product/" + productID))
                {
                    request.Headers.TryAddWithoutValidation("Authorization", "bearer " + bearerToken);
                    var response = await httpClient.SendAsync(request);
